package body Swan_Ada.Control.C_Interface is
   function Init_PID return PID_Double_Access is
      ...
      PID_Inner : constant PID_Controller_Access := new PID_Controller;
   begin
      PID.Inner := PID_Inner;
      return PID;
   end Init_PID;
   ...
   begin
      Set_Kp (Controller.Inner.all, Kp);
      Set_Ki (Controller.Inner.all, Ki);
      Set_Kd (Controller.Inner.all, Kd);
   end Set_Gains;

   procedure Set_Point_Inner (Controller : PID_Double_Access; SP : double) is
   begin
      Set_Point (Controller.Inner.all, SP);
   end Set_Point_Inner;
   ...
   is
      Value : double;
   begin
      Tick (Controller.Inner.all, Process_Variable, DT, Value);
      return Value;
   end Tick_Inner;
